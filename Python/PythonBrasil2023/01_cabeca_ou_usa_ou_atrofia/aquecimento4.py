...
print(len([p for p in txtB if p[0] in zumbi and p[-1] not in zumbi]))

verbos1p = 0
verbos = 0
for v in txtA:
    if len(v) == 7 and v[-1] not in zumbi:
        verbos = verbos + 1
        if v[0] not in zumbi:
            verbos1p = verbos1p + 1

print(verbos, verbos1p)

ordem = 'zmbtshjpnwlrcxkqvdgf'

def maiusculas(p):
    import string
    traduz = string.ascii_uppercase
    resp = ''
    for c in p:
        resp = resp + traduz[ordem.index(c)]

    return resp
        
print (maiusculas('cncdbm'))
print (sorted(txtA, key= maiusculas))

def num(p):
    p20 = 1
    soma = 0
    for c in p:
        soma = soma + ordem.index(c)*p20
        p20 = p20 * 20
    return soma
...
def magico(p):
    return num(p) %42 == 0 and len(p) == len(set(p))
for p in txtA:
    if magico(p): print(p)
print()
for p in txtB:
